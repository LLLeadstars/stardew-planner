...
/** v2～v6 都缺后续版本才引入的可选字段；补齐 playerStates、toolUpgrade 与 cropBatches 后再走当前校验。 */
function withUpgradeDefaults(state: unknown): PlannerState | null {
  ...
  const candidate = {
    ...
    cropBatches: Array.isArray(state.cropBatches) ? state.cropBatches : [],
  };
